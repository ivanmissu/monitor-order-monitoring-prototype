package com.minitor.server.config;
...
public class StoreConfig {
    ...
    @Bean
    @Primary
    public DataSource aggDataSource(MinitorProperties props) {
        return build(props.getStore().getAgg(), "minitor-agg", 16,
                "max_execution_time=10,max_memory_usage=2000000000");
    }

    /** cs_detail：只读 ods_order_event，长超时、高内存，但连接数严格受限。 */
    @Bean
    public DataSource odsDataSource(MinitorProperties props) {
        return build(props.getStore().getOds(), "minitor-ods", 4,
                "max_execution_time=60,max_memory_usage=6000000000,max_rows_to_read=500000000");
    }

    private DataSource build(MinitorProperties.Store.Ds ds, String poolName,
