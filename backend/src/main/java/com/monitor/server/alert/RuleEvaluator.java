...
public class RuleEvaluator {
    ...
    public RuleEvaluator(RuleRegistry registry, AggQueryService agg, MetricDictionary dict,
                         MonitorStore store, NoiseReducer noiseReducer, AlertEventBus bus,
                         NotifyService notify, MonitorProperties props) {
    ...
    @Scheduled(fixedDelayString = "${monitor.alert.eval-interval:60s}", initialDelay = 15_000)
    ...
    private void fire(NoiseReducer.Candidate c, int aggregatedCities) {
        ...
        MonitorStore.AlertRow row = new MonitorStore.AlertRow(id, c.ruleId(), c.level(), c.title(),
                ...
                List.of(new MonitorStore.AlertTimelineRow(now, "system", "fired",
                        "已按分级路由通知")));
        store.save(row);
    ...
    public Map<String, Object> preview(String ruleId, String lookback) {
        RuleRegistry.Rule rule = registry.find(ruleId)
                .orElseThrow(() -> com.monitor.server.common.ApiException.notFound("规则 " + ruleId));
