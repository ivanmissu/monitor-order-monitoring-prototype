...
public class NotifyService {
    ...
    public Map<String, Object> card(MonitorStore.AlertRow row) {
        ...
        card.put("links", List.of(
                Map.of("text", "下钻面板", "url",
                        "https://monitor.internal/redirect/drilldown?alert_id=" + row.alertId()),
                Map.of("text", "指标口径", "url",
                        "https://monitor.internal/dict/" + row.metricId() + "?v=" + row.metricVersion()),
     ...
     */
    public void dispatch(MonitorStore.AlertRow row) {
    ...
    private static String formatValue(MonitorStore.AlertRow row) {
        return row.denominator() > 0
                ? String.format("%.2f%%", row.value() * 100)
                : String.format("%.0f", row.value());
    }

    private static String formatBaseline(MonitorStore.AlertRow row) {
        return row.denominator() > 0
