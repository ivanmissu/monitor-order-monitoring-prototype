package com.monitor.server.engine;
 ...
 *       由 monitor 扫描「已送达但超时未入账」的订单产出，作为结算延迟率的分子；</li>
...
public class ScheduledJobs {
    ...
    public ScheduledJobs(MonitorStore store, IngestService ingest, AlertEventBus bus) {
        this.store = store;
        this.ingest = ingest;
        this.bus = bus;
    }
    ...
    public void scanSettlementOverdue() {
        ...
        IngestService.IngestResult r = ingest.ingest(overdue, "monitor-scanner");
    ...
    public void reconcileAndBackfill() {
        ...
        MonitorStore.ReconcileRow row = store.reconcile(dt);
        ...
        log.info("T+1 对账 dt={}: 状态缺口 {} 笔, 脏事件 {} 条, 缺失事件类型 {}",
                ...
                row.missingEvents().stream().map(MonitorStore.MissingEvent::eventType).toList());
    ...
    public void refreshLinkHealth() {
        List<MonitorStore.LinkNode> nodes = store.linkNodes();
