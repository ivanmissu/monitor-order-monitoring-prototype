# Minitor Server API 设计文档

**版本** v1.4.0 · **日期** 2026-09-03 · **状态** 评审稿
**基线**：`2026-09-03-carpool-monitor-prototype`（前端原型 8 个视图逐屏倒推）
**范围**：`minitor-server` 对外 HTTP/SSE 接口 —— 读侧查询（大盘 / 告警 / 明细 / 技术指标）+ 写侧事件接入。
...
```bash
curl -s "https://minitor.internal/api/v1/metric/query?\
...
X-Minitor-partial: true            # 当天分母仍在滚动（48h 匹配窗口 / T+1 回补）
X-Minitor-Grain: 1h
...
| Header | 示例 | 说明 |
|---|---|---|
| `X-Minitor-Freshness` | `2026-09-03T14:31:00+08:00` | 本结果覆盖的最新事件时间 |
| `X-Minitor-partial` | `true` | 1=未定盘，前端需打「滚动值」标记 |
| `X-Minitor-Grain` | `1m` | 实际生效粒度；小样本自动降级时返回降级后的粒度 |
| `X-Minitor-Dict-Version` | `2026.09` | 字典版本，口径一致性核对 |
...
| `admin` | `minitor_admin` | 全部 | 字典新版本、规则、回补、授权 | 平台研发 / 口径评审 |
